...
<?php 
$posts = $con->query("SELECT p.*,concat(u.firstname,' ',u.lastname) as name,u.profile_pic from posts p inner join users u on u.id = p.user_id  where p.id = {$_GET['id']}");
...
$commented =  $con->query("SELECT * FROM comments where post_id = {$_GET['id']} ")->num_rows;
?>
<style>
    ...
    <div class="col-md-5 h-100" style="overflow:auto">
        <div class="card card-widget post-card" data-id="<?php echo $id ?>">
            ...
            <div class="card-body">
                ...
                <br>
                <!-- <button type="button" class="btn btn-default btn-sm like <?php echo $is_liked ?>" data-id="<?php echo $_GET['id'] ?>"><i class="fa fa-check-circle"></i> Accept</button> -->
            ...
            <div class="card-footer card-comments">
				<?php 
					$comments = $con->query("SELECT c.*,concat(u.firstname,' ',u.lastname) as name,u.profile_pic FROM comments c inner join users u on u.id = c.user_id where c.post_id = {$_GET['id']} order by unix_timestamp(c.date_created) asc ");
