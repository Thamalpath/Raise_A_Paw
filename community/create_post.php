...
<?php
if (isset($_GET['id'])) {
	$qry = $con->query("SELECT * FROM posts where id = {$_GET['id']}")->fetch_array();
	foreach ($qry as $k => $v) {
		$$k = $v;
	}
}
...
<div class="container-fluid">
	<form action="" id="manage_post">
		...
		<div class="d-flex w-100 align-items-center">
			<div class="rounded-circle mr-1" style="width: 50px;height: 40px;top:-5px;left: -40px">
		...
		<div>
			...
			<div id="map">
				<iframe id="google_map" width="425" height="350" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.lk?output=embed "></iframe>
			</div>
		</div>
...
<div class="modal-footer display py-1 px-1">
	<div class="mt-3 mb-2">
		<button class="btn btn-hover w-100 color-9" form="manage_post" style="color: #fff; font-weight: 600;">POST</button>
	</div>
...
<script>
	var c = function(pos) {
		...
		document.getElementById('google_map').setAttribute('src', 'https://maps.google.lk/?q=' + coords + '&z=60&output=embed');
	}

	document.getElementById('get_location').onclick = function() {
		navigator.geolocation.getCurrentPosition(c);
		console.log(navigator.geolocation.getCurrentPosition(c));
		return false;
	}
</script>